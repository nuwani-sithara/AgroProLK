...
export default function ViewAllYieldsDetails() {
    ...
    // Fetch all yield details
    const getYields = () => {
        axios
            .get("http://localhost:8070/yieldsdetails/view-yieldsdetails")
            .then((res) => {
                setYields(res.data);
            })
            .catch((err) => {
    ...
    const handleRequestClick = (item, userEmail) => {
        // Navigate to the request page and pass the selected item data and userEmail
        navigate(`/request-yield`, { state: { yieldDetails: item, userEmail } });
    };
                    ...
                    <tbody className="tblbdy">
                        {yieldsdetails.map((item, index) => (
                            <tr className="tblrw" key={item._id}>
                                ...
                                <td>
                                    <button className="chkbtn" onClick={() => handleRequestClick(item, userEmail)}>Request</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
