...
export default function FertilizerCalculation() {
  ...
  const handlePlaceOrder = () => {
    ...
    if (confirmation) {
      ...
      axios.post('http://localhost:8070/fertilizerCalculation/calculate', {
        fertilizerId: fertilizer._id,  // Ensure you have the correct ID
        area: area,
        totalFertilizerNeeded: totalFertilizerNeeded,
        totalPrice: totalPrice,
      })
      .then((response) => {
        alert('Order placed and calculation saved!'); 
        navigate('/'); 
      })
      .catch((error) => {
